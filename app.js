...
function renderPortfolio(user) {
  ...
  user.portfolio.forEach(({ symbol, owned }) => {
    ...
    stockEl.innerHTML = `
      <p>Symbol: ${symbol}</p>
      <p>Shares: ${owned}</p>
      <button class="view-stock-btn" data-symbol="${symbol}">View</button>
    `;
  ...
  portfolioList.addEventListener('click', (event) => {
    if (event.target.classList.contains('view-stock-btn')) {
      viewStock(event.target.dataset.symbol);  // Display stock details for the clicked stock symbol
    }
  });
}
...
function viewStock(symbol) {
  ...
  if (stock) {
    ...
    document.querySelector('#stockPrice').textContent = `Price: $${stock.price || 'N/A'}`;
  } else {
    document.querySelector('#stockName').textContent = 'Stock not found';
    document.querySelector('#stockSector').textContent = '';
    document.querySelector('#stockIndustry').textContent = '';
    document.querySelector('#stockPrice').textContent = '';
  }
}
